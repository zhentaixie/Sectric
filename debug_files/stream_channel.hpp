...
public:
	...
	std::tuple<size_t, size_t> PrintStats() const;
	std::tuple<double, double,double> get_time_statistics() const;
};
...
NetIO::NetIO(std::string party, std::string address, int port)
{
	...
	else
	{
		...
		while (connect(this->connect_socket, (struct sockaddr *)&server_address, sizeof(struct sockaddr_in)) < 0)
		{
...
void NetIO::SendDataInternal(const void *data, size_t LEN)
{
	...
	while (HAVE_SENT_LEN < LEN)
	{
		...
		if (SENT_LEN >= 0)
			HAVE_SENT_LEN += SENT_LEN;
		else
			fprintf(stderr, "error: fail to send data %zu\n", SENT_LEN);
	}
	/*
	** very important:
...
void NetIO::ReceiveDataInternal(const void *data, size_t LEN)
{
	fflush(stream);
	auto start = std::chrono::high_resolution_clock::now(); // 开始时间
	auto start_with_wait = std::chrono::high_resolution_clock::now(); // 开始时间
	bool started = false;
	...
	while (HAVE_RECEIVE_LEN < LEN)
	{
		...
		if (RECEIVE_LEN > 0)
		{
			...
			HAVE_RECEIVE_LEN += RECEIVE_LEN;
		}else if(RECEIVE_LEN==0){}
		else
			fprintf(stderr, "error: fail to receive data %zu\n", RECEIVE_LEN);
	}
...
std::tuple<size_t, size_t> NetIO::PrintStats() const
{
	return std::make_tuple(totalBytesSent, totalBytesReceived);
}

std::tuple<double, double,double> NetIO::get_time_statistics() const
{
